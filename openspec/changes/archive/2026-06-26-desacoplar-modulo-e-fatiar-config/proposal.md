...
- `GoRouterModular.configure()` passa a orquestrar diretamente: `InjectionManager.registerAppModule(appModule)` + `ModularRouteBuilder(appModule).buildRoutes(topLevel: true)`.
