...
internal/asserts/...configure_assert.dart→ core/config(Modular,Modular), widgets(ModularApp)
routing/route_builder.dart               → core/config(Modular,RouteWithCompleterService), core/manager(InjectionManager), core/module(Module), exceptions(ModularException), internal(ModuleAssert), widgets(ModularLoader,OnceBuilder,ParentWidgetObserver)
