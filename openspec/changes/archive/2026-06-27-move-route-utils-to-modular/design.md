...
Em paralelo, a fachada `GoRouterModular` (`lib/src/bootstrap/go_router_modular_configure.dart`) já oferece leitura de estado por contexto via `getCurrentPathOf(context)` e `stateOf(context)` — ou seja, há **duplicação de conceito** entre extension e fachada, com nomes inconsistentes entre si (`getCurrentPathOf` vs `stateOf`).
...
**Goals:**
- Consolidar a leitura de estado da rota na fachada `GoRouterModular` com nomes consistentes no padrão `...Of(context)`.
...
### Decisão 1: Leitura de estado na fachada `GoRouterModular`, não em nova classe
Centralizar a leitura nos métodos estáticos de `GoRouterModular`, reaproveitando o ponto único que o consumidor já conhece (`Modular`/`GoRouterModular`). 
...
`getPathParam`, `getPath`, `state` na extension e `getCurrentPathOf`, `stateOf` na fachada recebem `@Deprecated('Use GoRouterModular.<novo> ...')`. Cada símbolo legado delega ao novo, evitando duplicação de lógica (DRY).
...
1. Adicionar os métodos novos em `GoRouterModular` (`routerStateOf`, `currentPathOf`, `pathParamOf` + wrappers `go_router`).
