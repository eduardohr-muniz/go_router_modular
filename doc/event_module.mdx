...
### 1. Configure Modular in main.dart

The EventModule system is configured through `Modular.configure()` in your `main.dart`:

```dart
void main() async {
  WidgetsFlutterBinding.ensureInitialized();

  // Configure Modular with EventBus settings
...
```dart
// Available EventBus configuration parameters in Modular.configure()
await Modular.configure(
...
_Built with ❤️ for the Flutter community_
