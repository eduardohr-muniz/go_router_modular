...
class EscolaModule extends Module {
  @override
  void binds(Injector i) {
    // Registrar Maria primeiro
    i.add<Maria>(() => Maria());

    // Registrar IPessoa SEM key para que seja acessível diretamente
    i.add<IPessoa>(() => Maria());

    // Registrar Escola que depende de IPessoa
    i.add<Escola>(() => Escola(aluno: i.get<IPessoa>()));
  }
}
...
class TrabalhoModule extends Module {
  @override
  void binds(Injector i) {
    // Registrar João primeiro
    i.add<Joao>(() => Joao());

    // Registrar IPessoa SEM key para que seja acessível diretamente
    i.add<IPessoa>(() => Joao());

    // Registrar Trabalho que depende de IPessoa
    i.add<Trabalho>(() => Trabalho(empregado: i.get<IPessoa>()));
  }
}
