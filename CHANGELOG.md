...
- **Dependency injection — self-referential interface factory (`addFactory<I>((i) => i.get())`)**: A common pattern in real apps is to bind multiple interfaces to a single concrete implementation by writing `i.addFactory<IFoo>((i) => i.get())` — the user expects `i.get()` to resolve to the registered concrete (`Impl`). Dart, however, infers the generic of `i.get()` from the surrounding context (the factory's return type), so `i.get()` collapses to `i.get<IFoo>()` and the resolver loops back into the same bind, throwing `Type "IFoo" is already being searched`. `BindLocator` now marks the bind whose factory is currently executing (per resolved type) and, on a recursive `i.get<T>()` for that type, skips that bind in both `_searchByType` and `_searchCompatibleBind`, falling through to the concrete impl indexed by its `runtimeType`. Covered by `test/untyped_get_interface_resolution_test.dart`.
