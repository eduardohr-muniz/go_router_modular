...
- **Animated** swap (aligned with **[go_transitions](https://pub.dev/packages/go_transitions)**) when any of those is set, or when you pass **`navigatorContainerBuilder`** — for example presets from **`StatefulShellBranchTransitions`** (`withGoTransition`, fade presets, …). The explicit builder **wins** over `transition`/durations.
